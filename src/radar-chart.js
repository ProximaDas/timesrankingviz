  ...
  chart: function() {
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        // Added by Proxima
        if (cfg.showLegend) {
          var chart_legend = container.selectAll('.legend-tiles').data(data);
              
              chart_legend.enter().append('rect')
                          .classed({'legend-tiles': 1, 'd3-enter': 1})
              chart_legend.exit()
              .classed('d3-exit', 1)
        ...
        /*
        var chart_legend = container.selectAll('.legend-tiles').data(data).enter()
        ...
        */
        var chart_labels = container.selectAll('.legend-labels').data(data).enter()
                                    .append("svg:text").classed("legend-labels",true)
                                    .attr("x",cfg.w - cfg.paddingX / 8 + (1.5*cfg.legendBoxSize))                            
                                    .attr("y", function(d,i) { return i*2*cfg.legendBoxSize; })
        ...
        polygon
          ...
          // styles should only be transitioned with css **CHANGED COLOR**
        ...
        if(cfg.circles && cfg.radius) {
          ...
          circle
            ...
            // styles should only be transitioned with css **CHANGE COLOR**
